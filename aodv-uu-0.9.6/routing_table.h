...
struct rt_table {
    ...
    // unicast transmission of AODV is exercised. Just knowing the next
    // hop is not enough; the routing agent must also consider which output
    // interface to be used to reach the next hop.
    // int rt_interface;
};
