    ...
    tqtimer(this), ifqueue(0)
{
    ...
    */
    ...
    // Author: Yg Song
    // MCMI: Initialize nIfaces
    nIfaces = 0;
    // Author: Yg Song
    // MCMI: Initialize fixed_interface
    fixed_interface = 0;
...
int NS_CLASS command(int argc, const char *const *argv)
{
    ...
    // Author: Yg Song
    // MCMI: Populate ifqueuelist and targetlist, take the corresponding values
    // while the interfacs are being created in the nodes.
    // argc is 4 if and only if mcmi.
    else if (argc == 4) {
        if (strcasecmp(argv[1], "if-queue") == 0) {
            ...
            ifqueuelist[temp_] = (PriQueue *)TclObject::lookup(argv[3]);
        ...
        // notable
            ...
            int temp_ = atoi(argv[2]);
            if (temp_ == nIfaces) nIfaces++;
            lllist[temp_] = (NsObject *)TclObject::lookup(argv[3]);
            if (lllist[temp_]) {
                return TCL_OK;
            }
            else {
